...
if [[ "$( wc -c </workspace/known_hosts )" -gt 0 ]]; then
  SSH_ARGS+=( -o UserKnownHostsFile=/workspace/known_hosts )

  [[ "${STRICT_HOST_KEY_CHECKING}" == "false" ]] && SSH_ARGS+=( -o StrictHostKeyChecking=false )
else
  [[ "${STRICT_HOST_KEY_CHECKING}" != "true" ]] && SSH_ARGS+=( -o StrictHostKeyChecking=false )
fi
...
for SERVER in "${SERVERS[@]}"; do
  ni log info "connecting to ${SERVER}..."
